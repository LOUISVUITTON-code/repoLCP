...
// DECLARAREMOS DOS FUNCIONES PARA LLAMARLAS EN EL MOMENTO QUE NECESITEMOS PARA NO REPETIR TANTO CODIGO

void primerTurno(carta mano[], int *puntos, int *contadorAs, int *num_cartas_mano, int *contadorMano);
void RobarCarta(carta mano[], int *puntos, int *contadorAs, int *num_cartas_mano, int *contadorMano);
...
int main()
{
  ...
  // DENTRO DEL IF IMPLEMENTAREMOS LA FUNCION DEL PRIMER TURNO

    if (turno == 0)
    {
      primerTurno(mano, &puntos, &contadorAs, &num_cartas_mano, &contadorMano);
    }
    cout << "\nIngrese una opcion: ";
    cin >> opcion;
    turno++;

    // EN LAS OPCIONES DEL SWITCH LLAMAREMOS A LAS FUNCIONES.

    switch (opcion)
    {
    case 1:
      RobarCarta(mano[], &puntos, &contadorAs, &num_cartas_mano, &contadorMano) 
      break;
...
// DEFINIMOS LA FUNCION DEL PRIMER TURNO.
...
// DEFINIMOS LA FUNCION DE ROBAR CARTA 
